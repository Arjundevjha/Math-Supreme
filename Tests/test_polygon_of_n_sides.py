...
class TestAreaOfPolygon(unittest.TestCase):
    def test_positive_integer_sides(self):
        ...
        self.assertTrue(abs(area_of_polygon(3, 2) - 1.7937923031667484) < 1e-9)

        # Test a square (n=4) with side=5
        self.assertTrue(abs(area_of_polygon(4, 5) - 25.083170062060734) < 1e-9)

    def test_positive_float_side(self):
        # Test a hexagon (n=6) with side=1.5
        self.assertTrue(abs(area_of_polygon(6, 1.5) - 5.846408408174998) < 1e-9)
